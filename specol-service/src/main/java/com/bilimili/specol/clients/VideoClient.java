...
public interface VideoClient {
    ...
    UserVideo getUserVideo(@RequestParam("sid") String sid, @RequestParam("vid") Integer vid);

    @PostMapping("/userVideo-service/insert/userVideo")
    @Operation(summary = "微服务接口：插入sid和vid的关系")
    void insertUserVideo(@RequestBody UserVideo userVideo);

    @PostMapping("/userVideo-service/update/userVideo")
    @Operation(summary = "微服务接口：改变sid和vid的关系")
    void updateUserVideoById(@RequestBody UserVideo userVideo);
}
